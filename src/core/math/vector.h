...
public:
    ...
    static Vector<D, T> Zero() { return Vector(); }
    ...
    [[nodiscard]] Vector<D, T> operator*(const Vector<D, T> &vec) const;
    [[nodiscard]] Vector<D, T> operator/(T k) const;
    [[nodiscard]] Vector<D, T> operator/(const Vector<D, T> &vec) const;
...
template <int D, typename T>
std::string Vector<D, T>::ToString() const
{
    std::stringstream os;
    os << "[ ";
    for (int i = 0; i < D; ++i)
